...
- `output/video_history.sqlite3`：热门视频素材历史（包含作者、分类、播放量、点赞量、文案、视频地址、封面地址和实际发布时间）。

各库的 `materials` 按素材编号去重，保留首次/最后发现时间；`observations` 按采集批次、国家、素材编号保存观测；`daily_snapshots` 视图按北京时间日期取同国同素材当天最后一次观测。重复导入同批次不增加记录，跨天不覆盖旧指标。已采数据每页入库，中断不丢失此前完成页面。首次发现是本库首次采到，不等于发布时间；热门视频会从其 TikTok 视频详情页的 `createTime` 读取实际发布时间，详情页未返回时保持“未获取”，不会根据视频 ID 推算。指标是素材整体指标，不按国家相加。
...
实测美国 20、日本 19、英国 19 条国家记录，共 53 个独立视频。国家按 Creator Studio 请求筛选记录，不代表作者所在地，也不是广告投放认证；不同市场可能返回重复热门内容，未验证的市场不承诺筛选完整性。默认日采遍历 28 个国家及全部 11 个分类，空结果、失败、重复页和页数上限都会记录。每个国家每天最多为 20 个此前未知的视频补全详情页发布时间；已取得的发布时间从本地素材索引复用，不重复请求。可通过 `video_detail_limit` 调整，设为 `0` 可关闭该补全。
