...
```bash
cd /opt/tiktok
bash deploy/provision-mihomo-proxy.sh
curl --proxy http://127.0.0.1:7897 https://www.gstatic.com/generate_204 -o /dev/null -w '%{http_code}\n'
```

The subscription URL and its returned node credentials must not be committed to the repository or written to cron logs.
